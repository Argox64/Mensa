import { GenerateRecipeRequest, NewRecipe, NewRecipeSchema, Recipe, RecipeListSchema, RecipePlannerRequest, RecipePlannerSchemaRequest, RecipeRequest, RecipeResponseSchema, RecipeSchema } from "@cook/validations";
...
const generateUniqueRecipePromptUser = (tags: string[], preparationTime: number, description: string) => `
  ### CONTRAINTES :
...
export async function generateUniqueRecipe({ ctx, input }: { ctx: ITRPCContext, input: GenerateRecipeRequest }): Promise<Recipe> {
    ...
    try {
        ...
        NewRecipeSchema.parse(generatedRecipe);
